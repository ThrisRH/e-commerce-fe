...
import { fetchProductByCateId } from "@/api/products/product-api";
...
const PartSelectionModal = ({
  ...
  const loadProducts = async () => {
    setLoading(true);
    try {
      const category = await fetchProductByCateId(categoryId);
      const items = (category || []).map((p) => new Product(p));
      setProducts(items);
    } catch (err) {
      ...
      {!selectedProduct ? (
        <>
          ...
          {loading ? (
            ...
            <Empty
              ...
              description="Chưa có sản phẩm trong hạng mục này"
        ...
        <div className="variant-selection">
          <Text type="secondary" style={{ display: "block", marginBottom: 16 }}>
            Vui lòng chọn một phiên bản linh kiện cụ thể:
          </Text>
          <Row gutter={[16, 16]}>
